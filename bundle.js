	...
	doSearch: function (val) {
		...
		if (val == "") {
			...
			for (var i in filteredData) {
				if (filteredData[i].paymentId.toLowerCase().indexOf(val) != -1 || filteredData[i].merchantName.toLowerCase().indexOf(val) != -1 || filteredData[i].merchantTransactionId.toLowerCase().indexOf(val) != -1 || filteredData[i].amountLeft.toLowerCase().indexOf(val) != -1 || filteredData[i].lastStatus.toLowerCase().indexOf(val) != -1 || filteredData[i].SNo.toLowerCase().indexOf(val) != -1) {
	...
	render: function () {
		...
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(Pagination, { len: tableData.length, pageSize: this.props.pageSize, tableData: tableData })
			)
		);
	}
});
...
var SearchBar = React.createClass({
	...
	render: function () {
		return React.createElement(
			...
			React.createElement('input', { type: 'text', placeholder: 'Enter Merchant Name,Order ID,Payment ID,Amount,Status', id: 'searchtext', className: 'form-control', onChange: this.doSearch })
		);
	}
});
	...
	render: function () {
		...
		if (page.currentPage > 1) {
			if (page.currentPage > 2) {
				...
				PagButtons.push(React.createElement(
					'span',
					{ className: 'PagButtons', 'data-val': '1', onClick: page.handleClick(1) },
					'«'
				));
				PagButtons.push(' ');
			}
			PagButtons.push(React.createElement(
				'span',
				{ className: 'PagButtons', 'data-val': page.currentPage - 1, onClick: page.handleClick(page.currentPage - 1) },
				'‹'
			));
			PagButtons.push(' ');
		}
		...
		if (page.currentPage < page.numPages) {
			...
			if (page.currentPage < page.numPages - 1) {
				...
				PagButtons.push(React.createElement(
					'span',
					{ className: 'PagButtons', 'data-val': page.numPages, onClick: page.handleClick(page.numPages) },
					'»'
				));
			}
		}
		return React.createElement(
			...
			React.createElement(
				'div',
				{ className: 'paginatedDiv' },
				PagButtons
			),
			React.createElement(DisplayTable, { tableData: page.data })
		);
	}
});
...
ReactDOM.render(React.createElement(SearchableTable, { tableData: result['content'], pageSize: 5 }), document.getElementById('main'));
